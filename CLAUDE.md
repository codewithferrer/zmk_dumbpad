...
The keymap is single-purpose: it turns the pad into a dedicated control surface for **Claude on Mac** — Claude Desktop chat shortcuts plus Claude Code's numbered permission-prompt digits (Deny/Allow once/Allow always) — not a general-purpose macropad. There is deliberately only one layer — don't add layers/`&mo` back in without checking that's actually wanted, since the last redesign explicitly removed the numpad/adjust/desktop-shortcut layers this board used to have.
...
- `1`/`2`/`3` (plain digit keys) select the corresponding option in a Claude Code permission prompt (Deny/Allow once/Allow always) — send the raw digit rather than a fixed semantic binding, since which action sits at which position shifts between the 2-option and 3-option prompt variants; the on-screen prompt is the source of truth for what a given digit does at any moment.
- `Cmd +`/`Cmd -` (zoom in/out) — confirmed real Claude Desktop shortcuts.
...
The v1.1 PCB has 3 onboard LEDs (QMK's `LED_00`/`LED_01`/`LED_02`, right/center/left), wired to `pro_micro` pins 10/15/14 respectively — derived from the original ATmega32u4 pins (`B6`/`B1`/`B3`) via the same pin-numbering table already validated against this repo's matrix/encoder wiring. They're driven by ZMK's built-in `zmk,indicator-leds` feature (no custom C code, no extra Kconfig): a `leds` node (`compatible = "gpio-leds"`) defines the 3 physical LEDs, and an `indicators` node (`compatible = "zmk,indicator-leds"`) maps each one to a standard HID lock indicator — right→Num Lock, center→Scroll Lock, left→Caps Lock. If a LED lights inverted, flip that LED's `GPIO_ACTIVE_HIGH` to `GPIO_ACTIVE_LOW` in the `leds` node — nothing else needs to change.

**The right/center LEDs (Num Lock/Scroll Lock) are currently orphaned.** Their toggle keys were repurposed to send plain `1`/`2` for Claude Code's permission prompts (see keymap section above), so nothing in this keymap ever changes the Num Lock/Scroll Lock host state anymore — those two LEDs just stay wherever they last were (almost certainly off) and won't visibly react to anything. The `indicators` devicetree nodes are harmless dead code, not a bug — only the left LED (Caps Lock, still real and host-driven) is currently live. If you want the right/center LEDs meaningful again, either bind a key to `&kp KP_NUM`/`&kp SLCK` somewhere, or repoint those two `indicator` child nodes at something else.
